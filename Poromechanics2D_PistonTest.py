...
endt = int(startT+100)*24*60*60 

imfold = 'PM_test'

tol=1e-4

Params = np.load(r"Parameters/piston.npz")
K = Params['K']
K[:1500//5,:] = 5e-17
K[Nx-1500//5:,:] = 5e-17
phi0 = Params['phi0']
...
for i in range(1075//5,1175//5):
    coords.append((1500//5,i))
    cells.append(Nx*(i)+1500//5)
    
    coords.append((Nx-1-1500//5,i))
    cells.append((Nx-1)+Nx*(i)-1500//5)
...
ux_0 = linalg.spsolve(J,-R)[:(Nx+1)*(Ny+1)]
uy_0 = linalg.spsolve(J,-R)[(Nx+1)*(Ny+1):]

Kfl = ((1-S_n)*cf_a + S_n*cf_b).reshape((Nx,Ny),order='F')       
Ksat = calc_Ksat(K_ave,K0,Kfl,phi.reshape((Nx,Ny),order='F'))
rhoT0 = (1-phi)*rhoR + mask*phi*(((1-S_n)*rho_a + S_n*rho_b)).reshape((Nx,Ny),order='F')         
cp0 = np.sqrt((Ksat+(4/3)*G_ave)/rhoT0)
...
while t < endt:
    ...
    while True:     
        ...
        if np.linalg.norm(R) < tol:         
            ...
            cp = np.sqrt((Ksat+(4/3)*G_ave)/rhoT)
